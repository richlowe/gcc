...
output_call_frame_info (int for_eh)
{
  ...
  if (fde_table_in_use == 0)
    {
#ifdef __linux__
      /* However, when output Sun IR, emit DW.ref.__gxx_personality_v0
         if any FDE is generated by Sun backend. 
         This is not a problem on Solaris because DW.ref. is not used. */
      per_encoding = ASM_PREFERRED_EH_DATA_FORMAT (/*code=*/2, /*global=*/1);
      if (eh_personality_libfunc 
          && ir_function_count > 0 
          && (per_encoding & DW_EH_PE_indirect))
        {
          dw2_force_const_mem (eh_personality_libfunc, 1);
        }
#endif
      return;
    }
