...
/*
 * XXXARM: AArch64 isn't bi-arch, and the gcc config is confusing, so _for
 * now_ we've taken config/sol2.h, as appropriate, into this file, and removed
 * it from the aarch64-solaris2 config.
 */
...
#define LINK_ARCH64_SPEC_BASE LINK_ARCH32_SPEC_BASE /* XXXARM: We're not multilib */

#undef LINK_ARCH64_SPEC
/* FIXME: Used to be SPARC-only.  Not SPARC-specfic but for the model name!  */
#define LINK_ARCH64_SPEC \
  "%{mcmodel=medlow:-M /usr/lib/ld/map.below4G} " \
...
/* Register the AArch64 and Solaris-specific #pragma directives.  */
#undef REGISTER_TARGET_PRAGMAS
#define REGISTER_TARGET_PRAGMAS() do { aarch64_register_pragmas(); solaris_register_pragmas (); } while (0)
