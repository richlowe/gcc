...
unsigned HOST_WIDE_INT first_rtl_backend = 0; 
unsigned HOST_WIDE_INT last_rtl_backend = 0;
unsigned HOST_WIDE_INT current_function_number = 0;
unsigned HOST_WIDE_INT ir_function_count = 0;

/* True if stabs/dwarf2 debugging information will be generated by dbg_gen.
   Ignored for other debugging formats.  Enabled by default, but disabled 
   when using "-frtl-backend" option with no args.  */
bool flag_use_dbg_gen = true;
...
bool
gate_generate_rtl (void)
{
  return !gate_generate_ir ();
}

static unsigned int
execute_generate_ir (void)
{
  struct eh_status *tmp;
  unsigned int builtin_return_addr_called;

  /* Let RTL backend handle this function if the __rtl_backend__
     attribute is specified, or the current_function_number falls
     in the [first_rtl_backend..last_rtl_backend] range.
   */
  if ((current_function_number <= last_rtl_backend
       && first_rtl_backend <= current_function_number)
      || lookup_attribute ("rtl_backend", 
  ...
  /* Save and restore all modified GIMPLE tree EH related nodes,
     bacause sometimes we need to switch back to RTL codegen.  */
  tmp = save_cfun_eh_status ();
  builtin_return_addr_called = cfun->builtin_return_addr_called;
  dump_function_ir (current_function_decl);
  if (!gate_generate_ir ())
    {
      cfun->builtin_return_addr_called = builtin_return_addr_called;
      restore_cfun_eh_status (tmp);
    }
#ifdef __linux__
  else
    {
...
tree_rest_of_compilation (tree fndecl)
{
  ...
  input_location = saved_loc;
   ...
   /* if IR gen was disabled for current function, enable it for next */
